...
# open cv _triangulate
def _triangulate(l_proj_matrix,r_proj_matrix,l_point,r_point):
    ...
    for index in range(0, 33):
        ...
        tmp = triangulation[0: -1] / triangulation[-1]
        cv_triangulation.append([tmp[0],tmp[1],tmp[2]])
...
def two_d_to_three_d_reconstruction(l_landmark,r_landmark,left_proj_matrix, right_proj_matrix):
    ...
    cv_triangulation_points = _triangulate(left_proj_matrix, right_projection_matrix, left_points, right_points)
...
if __name__ == "__main__":
    ...
    for left_image, right_image in zip(imagesLeft,imagesRight):
        ...
        print("CV Shape:" , cv_triangulation_points.shape)
        print("DLT Shape:" ,dlt_triangulation_points.shape)
        print("CV Points:", cv_triangulation_points)
        print("DLT Points:", dlt_triangulation_points)
        break
    lme.kill_processes()